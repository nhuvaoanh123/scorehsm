...
fn test_activation_same_counter_rejected() {
    ...
    assert!(ids
        .events()
        .iter()
        .any(|e| e.contains("ActivationRejected")));
}
...
fn test_activation_counter_mismatch_in_signature_rejected() {
    ...
    assert!(
        result.is_err(),
        "counter mismatch in signature must be rejected"
    );
    assert!(ids
        .events()
        .iter()
