...
fn make_leaf_cert_der(root_kp: &rcgen::KeyPair) -> Vec<u8> {
    ...
    params.signed_by(&leaf_kp, &root_cert, root_kp).unwrap().der().to_vec()
}
...
fn test_cert_parse_der_valid() {
    ...
    assert!(parse_der(&der).is_ok(), "valid DER cert must parse successfully");
}
...
fn test_cert_extract_ec_public_key() {
    ...
    assert_eq!(pk.len(), 65);
    assert_eq!(pk, expected_pk, "extracted public key must match known scalar");
}
...
fn test_cert_verify_chain_single() {
    let (der, root_pk) = make_root_cert_der();
    let cert = parse_der(&der).unwrap();
    assert!(verify_chain(&[cert], &root_pk).is_ok(), "single self-signed cert must verify against trust anchor");
}
...
fn test_cert_verify_chain_two_level() {
    ...
    assert!(verify_chain(&[leaf, root], &root_pk).is_ok(), "two-level chain must verify");
}
...
fn test_cert_verify_chain_empty_rejected() {
    ...
    assert!(verify_chain(&[], &root_pk).is_err(), "empty chain must be rejected");
}
